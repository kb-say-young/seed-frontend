...
<template>
  <div class="min-h-svh bg-surface-subtle">
    ...
    <main id="main" class="space-y-4 px-5 pb-10 pt-2">
      ...
      <section v-if="m.why" class="rounded-2xl bg-primary-tint p-4">
        <h2 class="text-label text-primary-dark">왜 먼저 해야 하나요?</h2>
        <p class="mt-1.5 text-body-sm leading-relaxed text-body">{{ m.why }}</p>
      </section>

      <section v-if="m.flowType === 'mixed' && m.goalAmount" class="rounded-2xl border border-border bg-surface p-4">
        ...
        <div class="mt-2.5 h-2 w-full overflow-hidden rounded-full bg-surface-subtle" aria-hidden="true">
          <div class="h-full rounded-full bg-amber" :style="{ width: `${m.progress ?? 0}%` }" />
        </div>
