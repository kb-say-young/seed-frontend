...
<template>
  <div class="flex min-h-svh flex-col bg-surface">
    ...
    <main id="main" class="flex-1 space-y-5 px-5 pt-2">
      <div class="flex rounded-xl bg-surface-subtle p-1" role="radiogroup" aria-label="유형">
        <button
          ...
          :class="kind === k ? 'bg-surface text-ink shadow-sm' : 'text-muted'"
          @click="kind = k"
        >
          {{ k === 'expense' ? '지출' : '수입' }}
        </button>
    ...
    <div
      class="sticky bottom-0 bg-surface px-5 pb-7 pt-3"
