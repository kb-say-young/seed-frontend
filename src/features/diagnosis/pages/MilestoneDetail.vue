...
const { data: m, loading, error, reload } = useResource(
  () => roadmapApi.getMilestone(String(route.params.id)),
  { requireAuth: true },
)
      ...
      <template v-else>
        ...
        <section v-if="m.flowType === 'mixed' && m.goalAmount" class="glass rounded-2xl p-4">
          ...
          <p class="mt-2 text-caption text-muted">
            목표 {{ won(m.goalAmount) }} · 남은 {{ won(remaining) }}
            <template v-if="m.targetDate"> · 이 속도면 {{ m.targetDate }} 달성</template>
          </p>
        </section>

        <section v-if="m.flowType === 'mixed' && m.checklist.length" class="glass rounded-2xl p-4">
          <h2 class="text-label text-ink">지금 할 일</h2>
          <ul class="mt-1 divide-y divide-border">
            <li v-for="c in m.checklist" :key="c.id" class="flex items-center gap-3 py-2.5">
              <span
              ...
              <span class="min-w-0 flex-1">
                ...
                <span v-if="c.plannedAmount > 0" class="mt-0.5 block text-caption text-muted">
                  예상 비용 {{ won(c.plannedAmount) }}
                </span>
              </span>
            </li>
          </ul>
        ...
        <section v-if="m.policy">
          ...
          <PolicyCard
            ...
            :reason="m.policy.reason ?? undefined"
