...
const props = withDefaults(
  defineProps<{
    ...
    errorMessage: string
    /** 비로그인이라 막힌 경우(`errorCode === AUTH_REQUIRED_ERROR_CODE`) — 재시도 대신 로그인으로 보낸다 */
    authRequired?: boolean
    /** 본문 섹션 간 간격. 기본 space-y-4 */
    spacing?: 'sm' | 'md'
  }>(),
  { spacing: 'md', authRequired: false },
)
  ...
  <div class="min-h-svh bg-transparent px-6 pb-32 pt-14">
    ...
    <main id="main" class="mt-8" :class="spacing === 'sm' ? 'space-y-3' : 'space-y-4'">
      ...
      <div v-else-if="showError" class="py-16 text-center">
        ...
        <UiButton v-if="authRequired" class="mt-3" @click="router.push('/login')">로그인하러 가기</UiButton>
        <UiButton v-else variant="secondary" class="mt-3" @click="emit('reload')">다시 시도</UiButton>
      </div>
